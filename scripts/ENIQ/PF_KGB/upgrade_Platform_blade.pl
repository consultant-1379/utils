...
my $BIN_PATH = "/proj/eiffel013_config_fem5s11/eiffel_home/bin";
my $KGB_DATA_PATH = "/proj/eiffel013_config_fem5s11/eiffel_home/FULL_PF_KGB_DATA/";
...
sub Transfer_Packages
{
	...
	$exp=Expect->spawn("/usr/bin/ssh root\@$HOST -C '/usr/bin/mkdir -p /tmp/PF_KGB/;/usr/bin/rm -rf /tmp/PF_KGB/*'") or die "Can not SSH and Clean $PLATFORM_DEST_PATH $! \n";
    ...
    $exp=Expect->spawn("/usr/bin/scp install_PF_eniq.sh getPkgs_list.py root\@$HOST:/tmp/PF_KGB/ ") or die "Can not scp to eniqs $! \n";
    if($exp->expect($timeout, 'assword:')){$exp->send('shroot12' . "\n");}
    $exp->expect($timeout, '#:');
    $exp->soft_close();
	...
	$exp=Expect->spawn("/usr/bin/ssh root\@$HOST") or die "Can not SSH\n";
	if ($exp->expect($timeout, 'Are you sure you want to continue connecting (yes/no)?')) {$exp->send('yes' . "\n");}
	if($exp->expect($timeout, 'assword:')){$exp->send('shroot12' . "\n");}
	$exp->expect(30, '#:');
    ...
    $exp->send("/usr/bin/scp eniqdmt\@${hubAddress}:/proj/eiffel013_config_fem5s11/eiffel_home/FULL_PF_KGB_DATA/${CHECKIN_JOB_BUILD_NUMBER}/CHECK_IN_FILES/* /tmp/PF_KGB/\r");
    if ($exp->expect($timeout, 'Are you sure you want to continue connecting (yes/no)?')) {$exp->send('yes' . "\n");}
...
{	
	...
	$exp=Expect->spawn("/usr/bin/scp root\@$HOST:/var/tmp/log /tmp/") or die "Can not SSH and Clean $PLATFORM_DEST_PATH $! \n";
	if ($exp->expect($timeout, 'Are you sure you want to continue connecting (yes/no)?')) {$exp->send('yes' . "\n");}
	if($exp->expect($timeout, 'assword:')){$exp->send('shroot12' . "\n");}
	$exp->expect($timeout, '#:');
	$exp->soft_close();
