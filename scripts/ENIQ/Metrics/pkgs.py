...
if len(str(day)) == 1:
	 ...
	 today1 = datetime.datetime.now().strftime ("%b ")+str(day)
#today = '%s-03-2019' %sys.argv[1]
#today1 = "Mar %s" %sys.argv[1]
#today2 = "%s-Mar-19" %sys.argv[1]
#week = datetime.date(2019,03,int(sys.argv[1])).isocalendar()[1]
...
def infra_checkins_count():
	os.chdir('/proj/eiffel004_config_fem167/slaves/RHEL_ENIQ_STATS/metrics/infra_spartan')
	p = subprocess.Popen("git rev-list --since=%s origin/%s | wc -l" %(today2,sys.argv[1][0:-6]), stdout=subprocess.PIPE, shell=True)
...
f = open("/proj/eiffel004_config_fem167/slaves/RHEL_ENIQ_STATS/metrics/config.py","w+")
...
f = open("/proj/eiffel004_config_fem167/slaves/RHEL_ENIQ_STATS/metrics/graph_template.html",'r')
content = f.read()
...
f = open("/proj/eiffel004_config_fem167/slaves/RHEL_ENIQ_STATS/metrics/graph_template_team.html",'r')
content = f.read()
...
f = open("/proj/eiffel004_config_fem167/slaves/RHEL_ENIQ_STATS/metrics/graph_template_checkins_team.html",'r')
content = f.read()
