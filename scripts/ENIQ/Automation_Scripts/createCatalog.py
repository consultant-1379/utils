...
def postCDB(datacenter, slave, addCatalog, catalogName):
    ...
    cloudUrl = 'https://' + str(username) + ':' + str(password) + '@' + str(sppPortal) + '.athtem.eei.ericsson.se/Vapps/index/orgvdc_id:'
    ...
    os.system('curl -u '+str(username)+':2021\#Mar --insecure '+'https://'+str(sppPortal)+'.athtem.eei.ericsson.se/Vapps/index_api/orgvdc_id:'+cloudDetails[cloudArea]['cloudArea'] + '/.xml > vappList.xml')
    ...
    os.system('curl -u '+str(username)+':2021\#Mar --insecure '+ '@' + str(sppPortal) + '.athtem.eei.ericsson.se/Vms/vapp_index_api/vapp_id:' + str(vappId) + '/.xml > vmList.xml') 
    document = ElementTree.parse("vmList.xml")
    for vms in document.findall('vms'):
        if vms.find('name').text == 'master_eniqs':
    ...
    cloudUrl = 'https://' + str(sppPortal) + '.athtem.eei.ericsson.se/Vms/set_memory_mb/vm_id:' + str(vmId) + '/vapp_id:' + str(vappId) + '/orgvdc_id:' + cloudDetails[cloudArea]['cloudArea'] 
    ...
    if (addCatalog == "True"):
        ...
        if failureStatus:
            ...
            os.system('curl -u '+str(username)+':'+str(password)+' --insecure '+'https://'+str(sppPortal)+'.athtem.eei.ericsson.se/VappTemplates/index_api/catalog_name:' + str(cloudArea) + '/org_id:' + str(catalogDetails[cloudArea]['cloudArea']) + '/.xml > catalogList.xml') 
...
def main():
    ...
    if not options.cloudArea or not options.jenkinsSlave or not options.addToCatalog or not options.catalogName:
