...
def genEnvVar():
    try:
		...
		aomNumber = urllib2.urlopen("https://ci-portal.seli.wh.rnd.internal.ericsson.com/getAOMRstate/?product=" + str(product) + "&drop=" + str(drop)).read()
        ...
        if scheduleVersion == "RELEASE":
            ...
            #tmpData = urllib.urlretrieve("https://arm1s11-eiffel013.eiffel.gic.ericsson.se:8443/nexus/content/repositories/releases/com/ericsson/eniq/stats/ci/eniq_taf_scheduler/maven-metadata.xml","tmpPom.xml")
            os.system("curl -u esjkadm100:Naples!0512 -O https://arm1s11-eiffel013.eiffel.gic.ericsson.se:8443/nexus/content/repositories/releases/com/ericsson/eniq/stats/ci/eniq_taf_scheduler/maven-metadata.xml")
            os.system("mv maven-metadata.xml tmpPom.xml")
        ...
        #tmpData = urllib.urlretrieve("https://arm1s11-eiffel013.eiffel.gic.ericsson.se:8443/nexus/content/repositories/releases/com/ericsson/cifwk/taf/taf-run-maven-plugin/maven-metadata.xml","tmpPom.xml")
        os.system("curl -u esjkadm100:Naples!0512 -O https://arm1s11-eiffel013.eiffel.gic.ericsson.se:8443/nexus/content/repositories/releases/com/ericsson/cifwk/taf/taf-run-maven-plugin/maven-metadata.xml")
        os.system("mv maven-metadata.xml tmpPom.xml")
        document = xml.parse("tmpPom.xml")
        ...
        try:
            ...
            isoVer = urllib2.urlopen("https://ci-portal.seli.wh.rnd.internal.ericsson.com/getlatestisover/?drop=" + str(drops[0]) + "&product=" + str(product)).read()
        ...
        if "KGB" in phase:
            ...
            for packageTemp in packageList:
                ...
                tempData = urllib2.urlopen('https://ci-portal.seli.wh.rnd.internal.ericsson.com/dmt/getLatestPackageObj/?package=' + str(package) + '&version=' + str(packageVersion))
            ...
            testwareTmp = urllib2.urlopen("https://ci-portal.seli.wh.rnd.internal.ericsson.com/testware/getmapping/?packagelist=" + str(" ".join(packageList)))
            testware = str(testwareTmp.read())
            pomUrl = "https://ci-portal.seli.wh.rnd.internal.ericsson.com/testware/createpom/?packagelist=" + str(" ".join(packageList)) + "&pomversion=2.0.2&scheduleversion=" + str(tafVer) + "&manualversion=" + str(manualVer) + "&tafrunversion=" + str(runVer) +"&schedulegroup=com.ericsson.eniq.stats.ci&scheduleartifact=eniq_taf_scheduler&schedulename=" + str(cdb_schedule)
        else:
            testwareTmp = urllib2.urlopen("https://ci-portal.seli.wh.rnd.internal.ericsson.com/testware/getmapping/?product=" + str(product) + "&isoartifact=" + str(isoNo) + "&isoversion=" + str(isoVer))
            testware = str(testwareTmp.read())
            pomUrl="https://ci-portal.seli.wh.rnd.internal.ericsson.com/testware/createpom/?isoartifact=" + str(isoNo) + "&isoversion=" + str(isoVer) + "&pomversion=2.0.2&scheduleversion=" + str(tafVer) + "&manualversion=" + str(manualVer) + "&tafrunversion=" + str(runVer) + "&schedulegroup=com.ericsson.eniq.stats.ci&scheduleartifact=eniq_taf_scheduler&schedulename=" + str(cdb_schedule)
