...
my $PF_KGB_DATA_DIR = ${JENKINS_HOME}."/PLM_PF_KGB_DATA/";
...
my $CLEARCASE_VIEWTAG = "plm_platform_view";
	...
	if (@all_zip)
	{
		...
		foreach (@all_zip)
		{
			...
			if ( grep( /^$plat_value/, @pf_modules ) ) { 
				...
				if ($rel eq "18" and $num ge '2'){
					push (@pf_pkgs_checked_in,$plat_full_path);
					print "\n=====================================================================";  
					print "\nThe XID is $xid, PLM Platform Package Name is $plat_name, Shipment is $comment\n";
					print "\n=====================================================================\n";
