...
def old_check_branch(projects,branch):
...
def old_create_table(projects,journey_table_name):
    ...
    for project, branch in projects.items():
        ...
        response_build = requests.get('https://sonarqube.lmera.ericsson.se/api/qualitygates/project_status?projectKey='+project+'&branch='+branch, headers=headers, auth=('113e5fae4d562c6b79bca47a56870206c2cff446', ''))
        ...
        url = "https://sonarqube.lmera.ericsson.se/dashboard?branch="+branch+"&id="+project
...
nmi = ['nmiNASd','nmibootstrap','deployment-tool','nmiinstall']
#nmi2 = ['nmiinstall']

netan = ['network-analytics-pm-explorer','network-analytics-pm-alarming','network-analytics-nr-kpi-dashboard','network-analytics-pm-data','network-analytics-energy-report','network-analytics-volte','network-analytics-RNO-MRR','network-analytics-IMS-Capacity','Network-Analytics-Uplink-Interference','network-analytics-NR-and-LTE-Timing-Advance','Transport-MINI-LINK-Performance','Ericsson-Voice-over-NR','NR-SA-KPI-Dashboard','LTE-KPI-Dashboard','WCDMA_Dashboard','Configuration-Management-Consistency-Check','NR-Uplink-Interference']
...
#    no_branch = []
#    yes_branch = []
#    l = old_check_branch(no_branch1,l1[i].strip())
#    no_branch1 = []
#    no_branch1 = no_branch

#old_create_table(final_dict,"NMI")
