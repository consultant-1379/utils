...
#$exp->expect($timeout, '#:');
#$exp->send("sed -i 's/ddc_san_data_collection/#ddc_san_data_collection/g' /eniq/installation/core_install/etc/stats_eniq_stats_stagelist\r");
#$exp->expect($timeout, '#:');
#$exp->send("\r");
#$exp->expect($timeout, '#:');
#$exp->send("exit\r");
...
if ($exp->expect($timeout, "Enter IP address of DNS SERVER(s)")) {$exp->send("192.168.0.1\r");}
#$exp->expect($timeout, "Enter IP address of DNS SERVER(s)");
#$exp->send("192.168.0.1\r");

if ($exp->expect($timeout, "Enter DNS domain name")) {$exp->send("athtem.eei.ericsson.se\r");}
...
$exp->send("/net/ieatmws300/JUMP/ENIQ_STATS/ENIQ_STATS/FEATURE/\r");
...
if ($exp->expect($timeout, 'Enter password for DBA')) {$exp->send("Dba12#\r");}
if ($exp->expect($timeout, 'Re-Enter password for DBA')) {$exp->send("Dba12#\r");}
if ($exp->expect($timeout, 'Enter password for DC')) {$exp->send("Dc12#\r");}
if ($exp->expect($timeout, 'Re-Enter password for DC')) {$exp->send("Dc12#\r");}
if ($exp->expect($timeout, 'Enter password for DCBO')) {$exp->send("Dcbo12#\r");}
if ($exp->expect($timeout, 'Re-Enter password for DCBO')) {$exp->send("Dcbo12#\r");}
if ($exp->expect($timeout, 'Enter password for DCPUBLIC')) {$exp->send("Dcpublic12#\r");}
if ($exp->expect($timeout, 'Re-Enter password for DCPUBLIC')) {$exp->send("Dcpublic12#\r");}
if ($exp->expect($timeout, 'Enter password for ETLREP')) {$exp->send("Etlrep12#\r");}
if ($exp->expect($timeout, 'Re-Enter password for ETLREP')) {$exp->send("Etlrep12#\r");}
if ($exp->expect($timeout, 'Enter password for DWHREP')) {$exp->send("Dwhrep12#\r");}
if ($exp->expect($timeout, 'Re-Enter password for DWHREP')) {$exp->send("Dwhrep12#\r");}
if ($exp->expect($timeout, 'Enter password for DCUSER')) {$exp->send("Dcuser%12\r");}
if ($exp->expect($timeout, 'Re-Enter password for DCUSER')) {$exp->send("Dcuser%12\r");}
...
if ($exp->expect($timeout, 'ILO Node is Configured or not? [y/n]')) {$exp->send('n' . "\n");}
