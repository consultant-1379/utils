...
sub SCP_CPAN
{
	...
	$timeout=15;
	$exp=Expect->spawn("/usr/bin/scp CPAN_Modules.zip cpanInstallerScript_Linux.pl $user_root\@eniqs:/eniq/home/dcuser/ ") or die "Can not SCP to $Machine  \n";
...
sub CPAN_Install
{
	...
	my $chmod_command = "chmod 777 CPAN_Modules.zip cpanInstallerScript_Linux.pl ";
	## Change Permissions
	$exp->expect($def, ['#:', sub {$exp = shift; $exp->send("cd /eniq/home/dcuser/ \r");}]);
	$exp->expect($def, ['#:', sub {$exp = shift; $exp->send("$chmod_command \r");}]);
	### Install CPAN
	$exp->clear_accum();
    $exp->expect($def, ['#:', sub {$exp = shift; $exp->send("perl /eniq/home/dcuser/cpanInstallerScript_Linux.pl CPAN_Modules.zip \r");}]);
	CheckExitCode();
	$exp->expect($def, ['#:', sub {$exp = shift; $exp->send("\r");}]);
	$exp->expect(3);
    $exp->soft_close();
