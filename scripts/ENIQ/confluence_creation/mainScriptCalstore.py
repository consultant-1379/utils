...
#Subpage creation
heading = ['HP Rack DL380', 'Blade and Multi Blade', 'BUSINESS OBJECTS',
           'ENIQ OCS', 'NETWORK ANALYTICS SERVER']
#XML parser
...
def table_form(table):
    ...
    f=False
    subd  = {}
    for ent in table.getElementsByTagName("entry"):
        if ent.childNodes:
            table1.append(ent.childNodes[0].data)
...
def create_confpage(page_dtails, pid=None):
    ...
    for i in page_dtails.keys():
       ...
       if pid:
           PID = conf.create_child_page('CAL-'+i.replace('\n', ' '), pid, 'zpunvai', PASSWD)
       else:
           PID = conf.create_parent_page('CAL-'+i.replace('\n', ' '), 'zpunvai', PASSWD)
       if type(page_dtails[i]) == list:
		...
		conf.create_child_page('CAL-'+subele.replace('\n', ' '), PID, 'zpunvai', PASSWD)
for i in body[0].childNodes:
    if 'chl' in str(i):
        #print chl_tag(i, {})
        create_confpage(chl_tag(i, {}), '438294309')
