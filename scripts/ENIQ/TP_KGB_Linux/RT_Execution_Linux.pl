...
sub PARAMETERS
{
		...
		{
                                if (($Shipment !~ /^[0-9][0-9].[0-9].[0-9][0-9].EU[0-9]-[0-9]$/) and ($Shipment !~ /^[0-9][0-9].[0-9].[0-9].EU[0-9]$/))
                                {
...
sub Get_RT_Latest_Version
{
	...
	$exp=Expect->spawn("/usr/bin/ssh $user_root\@eniqs ") or die "Can not SSH to $Machine  \n";
		...
		#my $cmd_latest_rt_version = "/usr/bin/wget --user=statsjenki --password=2020\#Jul -O - http://150.132.181.253/view/www_eniq/vobs/ossrc/del-mgt/html/eniqdel/$Release/$fdm_version/SOLARIS_baseline.html | grep 'RT_KGB_Delivery' | grep clearcase | grep '.tar' ";
...
sub Get_RT_And_Execute
{
	...
	#my $rt_download = "/usr/bin/wget --user=statsjenki --password=July\@2019 -N http://150.132.181.253/view/www_eniq/vobs/ossrc/del-mgt/html/eniqdel/$Release/$fdm_version/$rt_version -P /eniq/home/dcuser";
	my $rt_download = "/usr/bin/wget --user=statsjenki --password=2020\#Jul -N http://150.132.181.253/view/www_eniq/vobs/ossrc/del-mgt/html/eniqdel/$Release/$fdm_version/$rt_version";
	chomp($rt_download);
...
sub CheckExitCode
{
        ...
        if($result ne 0){
                print "\n\n Exit code for the last Script/Command call was $result.\n\t";
                #exit(-1);
        }else{
...
#############################################################################
### Function to get the output file to Workspace Area.
sub Get_Results
{
	chdir($WORKSPACE) or die "Unable to Switch to $WORKSPACE directory \n";
	$timeout=20;
	$exp=Expect->spawn("/usr/bin/scp $user_root\@eniqs:/eniq/home/dcuser/ResultFiles/KGB_RESULT_* . ") or die "Can not SCP to $Machine  \n";
                if ($exp->expect($timeout, 'Are you sure you want to continue connecting (yes/no)?')) {
                                             $exp->send('yes' . "\n");
                                         }
                                        $exp->expect($timeout,
...
sub Get_HTML_Results
{
        chdir($WORKSPACE) or die "Unable to Switch to $WORKSPACE directory \n";
        $timeout=20;
        $exp=Expect->spawn("/usr/bin/scp $user_root\@eniqs:/eniq/home/dcuser/KGB_RT_Reports.zip . ") or die "Can not SCP to $Machine  \n";
                if ($exp->expect($timeout, 'Are you sure you want to continue connecting (yes/no)?')) {
                                             $exp->send('yes' . "\n");
                                         }
                                        $exp->expect($timeout,
