...
$exp->expect($timeout, '#:');
$exp->send("val1=`cat /eniq/admin/version/eniq_status | grep Shipment | cut -d\" \" -f2 | cut -d\"_\" -f4 | cut -d\".\" -f1`\r");
$exp->expect($timeout, '#:');
$exp->send("val2=`cat /eniq/admin/version/eniq_status | grep Shipment | cut -d\" \" -f2 | cut -d\"_\" -f4 | cut -d\".\" -f2`\r");
$exp->expect($timeout, '#:');
...
$exp->expect($timeout, '#:');
$exp->send("bash ./unpack_core_sw.bsh -a create -d /net/10.45.192.134/JUMP/ENIQ_STATS/ENIQ_STATS/\${shipment}/eniq_base_sw/ -p \${tmpFolder}\r");
...
$exp->expect($timeout, '#:');
$exp->send("bash ./upgrade_eniq_sw.bsh -A upgrade_platform_only -D /net/10.45.192.134/JUMP/ENIQ_STATS/ENIQ_STATS/\${shipment}/eniq_base_sw/ -P \${tmpFolder} -O /net/10.45.192.134/JUMP/OM_LINUX_MEDIA/OM_LINUX_0\${val1}_\${val2}/\${val} -q\r");
$exp->expect($def, '#:');

print "\nInstalling all Platform Packages..!!\n";
$exp->send("cd /var/tmp/upgrade/\${tmpFolder}/core_install/bin/\r");
$exp->expect($timeout, '#:');
$exp->send("bash /eniq/installation/core_install/bin/upgrade_eniq_sw.bsh  -A upgrade_feature_only -f  /net/10.45.192.134/JUMP/ENIQ_STATS/ENIQ_STATS/Features_21\${numToAlpha[\$num]}_\${shipment} -q\r");
