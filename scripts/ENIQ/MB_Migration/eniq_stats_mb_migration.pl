...
sub PARAMETERS {
        usage() if ( @ARGV < 1 or ! GetOptions('help|?' => \$help, 'rel|r=s' => \$rel, 'ship|s=s' => \$ship, 'llsv_no|n=s' => \$llsv_no, 'mach|m=s' =>  \$mach, 'sms_signum|si=s' => \$sms_signum) or defined $help );
...
sub ENIQBLADEDETAILS{
        ...
        foreach (@eniqServerdedetails){
                ...
                if ( "$hostname" eq "$MACHINE" ){
						...
						$ENIQ_LIC = "/net/159.107.177.74/eniq/eniq_build/license/ENIQ_S19.2";
...
sub MultibladeMigration{
		...
		bannerPrint("CHAPTER 2 : STARTING UPGRADE STAGE PREREQUISITES...");
		mig_prerequisites();
		bannerPrint("CHAPTER 2 : COMPLETED UPGRADE STAGE PREREQUISITES...");
		
		# 5.4.6 Mandatory Pre-Upgrade IQ check
		bannerPrint("CHAPTER 5.4.6 : STARTING MANDATORY PRE-UPGRADE IQ CHECK...");
		PreUpgradeMandatoryIQCheck();
		bannerPrint("CHAPTER 5.4.6 : COMPLETED MANDATORY PRE-UPGRADE IQ CHECK...");

		
		# 5.4.8 UNPACK CORE SOFTWARE
		bannerPrint("CHAPTER 5.4.8 : STARTING UNPACK CORE SOFTWARE ...");
		UnpackCoreSW();
		bannerPrint("CHAPTER 5.4.8 : COMPLETED UNPACK CORE SOFTWARE ...");
		
		# 5.4.9 Upgrade Pre-Execution Check
		bannerPrint("CHAPTER 5.4.9: STARTING UPGRADE PRE-EXECUTION CHECK ...");
		UpgradePreCheckExecutionProcedure();
		bannerPrint("CHAPTER 5.4.9: COMPLETED UPGRADE PRE-EXECUTION CHECK ...");
		
		#5.5.1 Run Pre-migration
		bannerPrint("CHAPTER 5.5.1: STARTING PRE-MIGRATION ...");
		Multiblade_Migration_PM_procedure();
		bannerPrint("CHAPTER 5.5.1: COMPLETED PRE-MIGRATION ...");
...
sub PreUpgradeMandatoryIQCheck{
	...
	$exp->send("cat /eniq/admin/etc/dbcheck.env | grep RUNNUMBER\r");
	print "preuograde\n";
...
sub UnpackCoreSW{
	...
	$exp->send("bash ./unpack_core_sw.bsh -a create -d $jumparea -p $upgradeDir\r");
