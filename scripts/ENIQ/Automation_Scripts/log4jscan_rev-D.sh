...
if [ \$ext != jar ]; then
  ...
  if [  \$? -eq 0 ]; then
    ...
    find . -type f -regextype posix-egrep -iregex ".*[.](jar|war|ear)" -print -exec unzip -l {} \; | awk -vinfile=\$infile '{ if (\$1=="Archive:") name=\$2 ; sub("./","",name);
		...
		if (index(file_to_check,"jndilookup")) found=1
        if (index(file_to_check,"jndimanager.class")) found=1
        if (index(file_to_check,"jndimanager")) found=1
        if (index(file_to_check,"jmsappender.class")) found=1
		if (index(file_to_check,"jmsappender")) found=1
		if (index(file_to_check,"chainsaw.class")) found=1
		if (index(file_to_check,"chainsaw")) found=1
		if (index(file_to_check,"socketserver.class")) found=1
		if (index(file_to_check,"socketserver")) found=1
		if (index(file_to_check,"JMSSink")) found=1
		if (index(file_to_check,"JMSSink.class")) found=1
		if (index(file_to_check,"jdbcappender")) found=1
		if (index(file_to_check,"jdbcappender.class")) found=1
