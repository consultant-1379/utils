...
my $KGB_DATA_PATH = "/proj/eiffel004_config_fem156/eiffel_home/PLM_PF_KGB_DATA/";
...
my $hubAddress = "150.132.181.179";
my $hubPassword = "EStAts(iDec\$2()18";
my $logfile="/var/tmp/PLM_PF_KGB_CONSOLE_LOG.txt";
my $PLATFORM_DEST_PATH = "/tmp/PLM_PF_KGB/";
...
sub Transfer_Packages
{
	...
	$exp=Expect->spawn("/usr/bin/ssh root\@eniqs -C '/usr/bin/mkdir -p /tmp/PLM_PF_KGB/;/usr/bin/rm -rf /tmp/PLM_PF_KGB/*'") or die "Can not SSH and Clean $PLATFORM_DEST_PATH $! \n";
    ...
    $exp=Expect->spawn("/usr/bin/scp install_PF_eniq.sh getPkgs_list_PF_PLM_track.py root\@eniqs:/tmp/PLM_PF_KGB/ ") or die "Can not scp to eniqs $! \n";
    if($exp->expect($timeout, 'assword:')){$exp->send('shroot12' . "\n");}
    $exp->expect($timeout, '#:');
    $exp->soft_close();
    ...
    $exp->send("/usr/bin/scp eniqdmt\@${hubAddress}:/proj/eiffel004_config_fem156/eiffel_home/PLM_PF_KGB_DATA/${CHECKIN_JOB_BUILD_NUMBER}/CHECK_IN_FILES/* /tmp/PLM_PF_KGB/\r");
    if ($exp->expect($timeout, 'Are you sure you want to continue connecting (yes/no)?')) {$exp->send('yes' . "\n");}
...
{	
	...
	$exp->expect($timeout, '#:');
	$exp->send("/usr/bin/python ${PLATFORM_DEST_PATH}getPkgs_list_PF_PLM_track.py \$shipment\r");
	$exp->expect(10);
