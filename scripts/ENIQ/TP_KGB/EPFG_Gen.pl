...
sub Get_Epfg_RT_Latest_Version
{
	...
	my $cmd_latest_epfg_rt_version = "/usr/sfw/bin/wget --user=statsjenki --password=2020\#Apr -O - http://clearcase-oss.lmera.ericsson.se/view/www_eniq/vobs/ossrc/del-mgt/html/eniqdel/$Release/$fdm_version/SOLARIS_baseline.html | grep epfg_rt | grep '.zip' ";
	...
	$epfg_version = `cat $get_epfg_rt_version_file | grep 'epfg_rt' | tail -1 | awk -F'">' '{print \$1}' | awk -F'/' '{print \$NF}'`;
	$epfg_version =~ s/\s+//g;
	if(!$epfg_version)
	{
...
sub Pre_Run_EPFG_RT
{
	...
	my $epfg_download = "/usr/sfw/bin/wget --user=statsjenki --password=2020\#Apr -N http://clearcase-oss.lmera.ericsson.se/view/www_eniq/vobs/ossrc/del-mgt/html/eniqdel/$Release/$fdm_version/$epfg_version -P /eniq/home/dcuser";
	$exp->expect($undef, $PROMPT);
	...
	print " >>>>>>>>>>>>>>> Downloading Completed <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n";
        ...
        my $inner_RT_Version_Name = `cat $get_inner_RT_Version_File | grep extracting | cut -d ':' -f2`;
		...
		$exp->expect($undef, $PROMPT);
                ...
                $exp->send("cd /tmp;/usr/bin/chmod 755 epfgdataGeneration.pl;perl /tmp/epfgdataGeneration.pl\r");
		$exp->expect($undef, $PROMPT);
		print "\n\n\n +++++++++++++++++++++++++++++++++++++++++++++++ STARTING EPFG ++++++++++++++++++++++++++++++++++++++ \n\n\n";
