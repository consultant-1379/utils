...
my $KGB_DATA_DIR = ${JENKINS_HOME}."/KGB_DATA_Sol/";
...
my $CLEARCASE_VIEWTAG = "tp_kgb_jenkin_view";
...
my @all_tpi=`grep /vobs/eniq $Polling_log_file | sort | uniq | sed s'/"//g'| awk '{print \$2 \$3}'| awk '!seen[\$0]++' | grep tpi | awk -F '/' '{print \$1","\$NF}' `;

my @tpi_full_paths = `grep /vobs/eniq $Polling_log_file | sort | uniq | sed s'/"//g' | awk '{print  \$3}' | awk '!seen[\$0]++' | grep tpi | cut -d'/' -f4- | awk '{print "/"\$0}'`;
	...
	if (@all_tpi)
	{
		...
		foreach (@all_tpi)
		{
			...
			if ("$tp_name" !~ /^EVENT/ )
			{
				...
				print "-----------------------------------------------------------\n";
				if( "$comment" =~ /^ENIQSTATS/ )
				{
	
					if( "$(split /_/,$comment)[0]" =~ "ENIQSTATS" )
					{
						#print "The pkg is a ENIQSTATS package\n";
						my $comm = (split /_/,$comment)[1];
						$comm =~ s/\s+//g;
						#print "The Comment is $comm original \n";
						if ( "$comm" !~ /(^WP|^H)/ )
						{
							$comm= "WP0000/".$comm;		
							print "The Comment for the package $tp_name is $comm \n";
						}		
						...
						if ( "$fd_TP_Name" !~ /^BO/ )
						{
							...
							$fd_check = `grep /vobs/eniq $Polling_log_file | sort | uniq | sed s'/"//g'| awk '{print \$3}'| awk '!seen[\$0]++' | grep $fd_TP_Name | grep 'xls\\|xls' | awk -F '/' '{ print \$NF }'| awk -F '_[0-9]' '{print \$1}' `;
						...
						chomp($fd_check_val);	
						print "\n=====================================================================";  
						print "\nThe XID is $xid, Tech Pack Name is $tp_name, Comment is $comm,FD Check Status is $fd_check_val\n";
						print "\n=====================================================================\n";
						open (Outputfile , ">> $output_file");
						print Outputfile "$xid===$tp_name===$comm===$fd_check_val\n";
					...
					{
						print "There are no proper comment's for the $tp_name Tech Pack. OR it might not be ENIQSTATS Tech Pack \n";
						push(@tp_comment_failed,$only_tp_name); # Push if it is not a valid comment w.r.to ENIQSTATS_<WP000/123456>
					}
...
sub Copy_TP
{
	...
	foreach (@tpi_full_paths)
        {
			chomp($_);
			my $tp_full_path= $_;
			my $tp_val = (split /\//,$_)[-1];
			my $tp_value = (split /_R[0-9]/,$tp_val)[0];
			chomp($tp_val);
			chomp($tp_value);
			$tp_value =~ s/ //g;
			if ( $tp_value !~ /^EVENT/ )
			{
				...
				{
               	 			print "Coping the Tech Pack $tp_val to $CHECK_IN_ALL path \n\n";
                			`/usr/atria/bin/cleartool setview -exec 'cp -rf $tp_full_path $CHECK_IN_ALL' ${CLEARCASE_VIEWTAG} `;
					`/usr/atria/bin/cleartool setview -exec 'cp -rf $tp_full_path $Delivering_Dir' ${CLEARCASE_VIEWTAG} `;
			...
			system("/usr/atria/bin/cleartool setview -exec 'perl /proj/eiffel004_config_fem160/eiffel_home/bin/Move_Tech_Pack.pl $CHECK_IN_ALL $CHECK_IN_FILES' ${CLEARCASE_VIEWTAG}");
...
sub Remove_TP_Failed
{
	...
	{	
		foreach (@tpi_full_paths)
			...
			{
				print "\n\nRemoving the Tech Pack $CHECK_IN_FILES$tp_val as it failed to validate Comments \n\n";
				`rm -rf $CHECK_IN_FILES$tp_val`;
			}
...
sub COPY_CSV_PM
{
	foreach (@NODE_NAMES_FOR_CSV_PM_FILES)
	{
		...
		if ($_)
		{
			...
			{
				$File =~ s/\s+//g;	
				$File =~ s/\/view\/statsjenkins_view//g;	
				chomp $File;
