...
def createProp():
	...
	file_object.write('\nReason = '+reason)
	file_object.close()
	if "ENIQ_Feature_Files" not in pack and "lcmbiar" not in pack:
		os.system("cp "+pack+" /proj/eiffel013_config_fem5s11/eiffel_home/Linux_KGB_DATA/Pack/")
		os.system("cp "+prop+" /proj/eiffel013_config_fem5s11/eiffel_home/Linux_KGB_DATA/Prop/")
	if "tpi" in pack or "zip" in pack:
		os.system("cp "+pack+" /proj/eiffel013_config_fem6s11/eiffel_home/jobs/TP_Vobs_Delivery/")
	if "xlsx" in pack:
		os.system("rm -rf /proj/eiffel013_config_fem6s11/eiffel_home/jobs/CounterToMOClassMApping/input/*")
		os.system("cp "+pack+" /proj/eiffel013_config_fem6s11/eiffel_home/jobs/CounterToMOClassMApping/input/")

def uploadCheck():
	fd = os.system("cat /proj/eiffel013_config_fem6s11/eiffel_home/jobs/TP_Nexus_Upload/builds/"+num+"/log| grep -i error")
	fc = os.system("cat /proj/eiffel013_config_fem6s11/eiffel_home/jobs/TP_Nexus_Upload/builds/"+num+"/log| grep -i curl:")
	#fb = os.system("cat /proj/eiffel013_config_fem6s11/eiffel_home/jobs/TP_Nexus_Upload/builds/"+num+"/log| grep -i manual")
	if fd == 0 or fc == 0:
...
def uploadnewtool():
        ...
        n=len(groups)
        split_pkg = '_'.join(groups[:n-1]), '_'.join(groups[n-1:])
        d = os.system("ls /proj/eiffel013_config_arm1s11/eiffel_home/storage/releases/com/ericsson/eniq/stats/tool_script/"+split_pkg[0]+"/ | grep "+split_pkg[1])
        if d == 0:
                print "Already present in Nexus "+pack
                os.system("rm "+pack)
                os.system("rm "+prop)
                ...
                d = os.system("curl -v -u esjkadm100:Naples\!0512 --upload-file "+pack+" https://arm1s11-eiffel013.eiffel.gic.ericsson.se:8443/nexus/content/repositories/releases/com/ericsson/eniq/stats/tool_script/"+split_pkg[0]+"/"+split_pkg[1]+"/"+pack)
                time.sleep(10)
                uploadCheck();
                d = os.system("curl -v -u esjkadm100:Naples\!0512 --upload-file "+prop+" https://arm1s11-eiffel013.eiffel.gic.ericsson.se:8443/nexus/content/repositories/releases/com/ericsson/eniq/stats/tool_script/"+split_pkg[0]+"/"+split_pkg[1]+"/"+prop)
                print "Verifying the uploaded package\n"
                time.sleep(10)
                uploadCheck();
