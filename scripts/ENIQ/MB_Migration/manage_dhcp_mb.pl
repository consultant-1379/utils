...
my $timeout = 20;
my $exp;
my $TEMP_FILE = "/tmp/Auto_DHCP_Log.txt";
...
sub add_dhcp_client
{
    ...
    $exp->expect(5, '#');
    $exp->send("find /JUMP/OM_LINUX_MEDIA/OM_LINUX_019_2/ -name \".om_linux_identity\" | sort > /tmp/om_location\r");
    $exp->expect(5, '#');
    ...
    if($exp->expect($timeout, 'Enter the IP address of ')){$exp->send("\r");}
    if($exp->expect($timeout, 'Enter the IP Netmask of ')){$exp->send("255.255.248.0\r");}
    if($exp->expect($timeout, 'Please enter the MAC address for ')){$exp->send("1C:98:EC:1B:72:CC\r");}
    if($exp->expect($timeout, 'Do you want IPV6 enabled - YES/NO?')){$exp->send("NO\r");}
    
    if($exp->expect($timeout, 'Enter the Timezone to be set for ')) {$exp->send("Eire\r");}
    ...
    print "\nAdding CLIENT_INSTALL_PARAMS value to ${server}_ks_cfg.txt..!!\n";
    $exp->expect(10, '#');
    $exp->send("echo \"CLIENT_INSTALL_PARAMS=inst_type=eniq config=stats deployment=ft\" >> /JUMP/LIN_MEDIA/2/kickstart/${server}/${server}_ks_cfg.txt\r");
    $exp->expect(5, '#');
    $exp->soft_close();
}
